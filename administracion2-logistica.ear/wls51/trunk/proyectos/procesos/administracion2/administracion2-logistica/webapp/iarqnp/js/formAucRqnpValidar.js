...
var btnAtras_click = function(){
	if(confirm("Confirme si desea Cancelar el Registro")){
		...
		document.frmRegistro.action="bandejaauc.htm?action=iniciarbandejaauc";
		document.frmRegistro.submit();
	}
}
...
var btnAtender_click = function () {
	...
	if (sw=='1'){
		...
		alert("Seleccione los ítems que desea Atender");
	}
};
	...
	var sw='0';
		...
		if (sw=='1'){
			if(confirm("Confirme si desea RECHAZAR los ítems Seleccionados")){
				dojo.byId("action").value = "rechazarValidarRqnp";
				var formDlg = dijit.byId("formDialogRechazo");
				formDlg.show();
			}
		}else{
			alert("Seleccione los �tems que desea Rechazar");
		}
};
